...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        '''
        nbr_states = []
        curmatrix = self.state
        curzero = self.zero_loc
        row = curzero[0]
        col = curzero[1]
        
        if (row != 2):
            newmatrix = copy.deepcopy(curmatrix)
            newmatrix[row][col] = newmatrix[row+1][col]
            newmatrix[row+1][col] = 0
            # print("curma: ",curmatrix)
            # print("nma1: ",newmatrix)
            new_state = EightPuzzleState(newmatrix,self.goal,dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic,zero_loc=(row+1,col))
            nbr_states.append(new_state)
        if (col != 0):
            newmatrix = copy.deepcopy(curmatrix)
            ...
            # print("curma: ",curmatrix)
            # print("nma2: ",newmatrix)
            new_state = EightPuzzleState(newmatrix,self.goal,dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic,zero_loc=(row,col-1))
            nbr_states.append(new_state)
        if (row != 0):
            newmatrix = copy.deepcopy(curmatrix)
            newmatrix[row][col] = newmatrix[row-1][col]
    ...
    def compute_heuristic(self):
        ...
        for i in range(9):
            ...
            col = np.where(matrix==i)[1][0]
            # print("curx y ", row,col)
            expectedx = np.floor(i/3) 
            expectedy = i%3
            total+=manhattan((row,col),(expectedx,expectedy))
