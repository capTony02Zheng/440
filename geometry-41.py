...
def does_alien_touch_wall(alien: Alien, walls: List[Tuple[int]]):
    ...
    if(circle):
        for line in walls:
            ...
            if(point_segment_distance(cen,l) <= width):
                return True
    else:
        for line in walls:
            l = tuple([(line[0],line[1]),(line[2],line[3])])
            head = headtail[0]
            tail = headtail[1]
            if( segment_distance(l,headtail) < width) :
                return True
...
def do_segments_intersect(s1, s2):
    ...
    if(max(x3,x4) < min(x1,x2) or max(y3,y4) < min(y1,y2) or max(x1,x2) < min(x3,x4) or max(y1,y2) < min(y3,y4) ):
        return False
    flag1 = ((x1-x3)*(y4-y3) - (y1-y3)*(x4-x3))* ((x2-x3)*(y4-y3) - (y2-y3)*(x4-x3))
    flag2 = ((x3-x1)*(y2-y1) - (y3-y1)*(x2-x1))* ((x4-x1)*(y2-y1) - (y4-y1)*(x2-x1))
